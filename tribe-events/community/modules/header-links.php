...
<header class="my-events-header">
	<h2 class="my-events">
		...
		if ( $post_id && tribe_is_event( $post_id ) ) {
			esc_html_e( 'レッスン・イベントを編集（Edit Event）', 'tribe-events-community' );
		} elseif ( $post_id && tribe_is_organizer( $post_id ) ) {
			esc_html_e( '主催者情報を編集（Edit Organizer）', 'tribe-events-community' );
		} elseif ( $post_id && tribe_is_venue( $post_id ) ) {
			esc_html_e( '会場情報を編集（Edit Venue）', 'tribe-events-community' );
		} else {
			esc_html_e( 'レッスン・イベントを追加（Add New Event）', 'tribe-events-community' );
		}
		?>
	</h2>
